...
function Server(options) {
  ...
  var defaults = {path: '/sse', fallback: true, keepalive: 1000, authenticationResolver: null, needsAuth: false, create: false}
  options = options || defaults
  options = typeof options === 'string' ? merge(defaults, { path: options }) : options
  options.keepalive = options.keepalive || defaults.keepalive
  options.fallback = typeof options.fallback !== 'undefined' ? options.fallback : defaults.fallback
