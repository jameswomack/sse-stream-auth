...
module.exports = {
  ...
  matcherTemplates: [ '%s', '%s/:channelName', '%s/:channelName/:message' ],


  defaultChannelName: 'sse',


  createMatchers : function () {
    // Reset matchers
    this.accept.matchers = [];

    this.accept.prefixes.forEach(function (prefix) {
      this.matcherTemplates.forEach(function (matcherTemplate) {
        this.accept.matchers.push(matcherTemplate.replace('%s', prefix));
      }.bind(this));
    }.bind(this));
  },
